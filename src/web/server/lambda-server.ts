...
export function startLambdaServer(
  port: number,
  ...
  return new Observable<LogItem>((obs) => {
    ...
    app.use(async (ctx) => {
      ...
      const fnPayload: FnPayload = {
        headers: {
          ...pickBy(ctx.req.headers, (_value, key) =>
...
function runRequest(
  ...
  fnPayload: FnPayload,
  executionTimeout: number,
  logCallback: (item: LogItem) => void,
  inspect?: boolean
): Promise<ResponseType["data"]> {
  return new Promise((resolve, reject) => {
    const child = child_process.fork(
      path.join(WEB_BASE_PATH, "/assets/lambda-dev-wrapper"),
      [],
      {
        cwd: projectPath,
        stdio: ["pipe", "pipe", "pipe", "ipc"],
        execArgv: inspect ? ["--inspect-brk=14396"] : [],
      }
    );
    if (!child) {
    ...
    child.stdout?.on("data", (out: string) =>
      logCallback({ type: "out", value: out })
    );
    child.stderr?.on("data", (out: string) =>
      logCallback({ type: "err", value: out })
    );
