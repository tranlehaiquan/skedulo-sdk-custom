...
export class DebugInstall extends React.PureComponent<IProps> {
  renderItem = (key: string, item: DebugState | null) => {
    if (!item) {
      return (
        <li>
          <i className="icon sk-loader-small" />
          {key}
        </li>
      );
    }
    ...
    if (item.valid) {
      return (
        <li>
          <i className="icon color-green ski ski-tick" />
          {key}
        </li>
      );
    } else {
      return (
        <li>
          <i className="icon color-red ski ski-remove" />
          {key} {item.reason}.{" "}
          {item.link && (
            <span>
              <br />
              <a className="blue-link" onClick={() => openUrl(item.link!)}>
                Install Now
              </a>
            </span>
          )}
      ...
      <div className="text-left padding-top installation__container">
        ...
        <p>
          ...
          <a
            className="blue-link"
            onClick={() => {
              this.props.setView && this.props.setView(View.SetupSSL)();
            }}
          >
            here
          </a>
