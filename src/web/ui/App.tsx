...
export class App extends React.Component<{}, IState> {
  ...
  appInitialize() {
    ...
    if (!this.state.sslCertsPresent) {
      ...
      this.subscription.add(
        this.checkDevelopmentEnvironment().subscribe(void 0, (error) =>
          console.error(error)
        )
      );
    } else {
      const onNewProject$ = this.eventChannel
        .onNewProject()
      ...
      this.subscription.add(
        Observable.merge(
          ...
          this.listenForDevelopmentSession()
  ...
  setView = (currentView: IState["currentView"]) => () =>
    this.setState({ currentView });

  refreshPackage = async (
    goToConfiguration: boolean,
    refreshLinks: boolean = false
  ) => {
    ...
    try {
      ...
      const refreshedPackage = PackageService.at(
  ...
  goHome = () =>
    this.setView(this.state.sslCertsPresent ? View.Home : View.SSLNotSetPrompt);

  selectConnectedPageProject = (selectedProject: string) => {
    if (!fs.existsSync(path.join(selectedProject, "/sked.proj.json"))) {
      this.setState({
        errorMessage:
          "The folder you have selected does not contain a valid Standalone Web Extensions project file. Please select another.",
      });
    } else {
      this.setState({
  ...
  checkDevelopmentEnvironment = () => {
    ...
    const setter =
      (namespace: keyof IState["environment"]) => (status: DebugState) =>
        this.setState({
          environment: {
  ...
  listenForDevelopmentSession = () => {
    return this.eventChannel.onSessionEvent().do((session) => {
      if (session) {
        ...
        fetch(session.API_SERVER + "/custom/usermetadata", {
          ...
          .then((response) => {
            if (response.status === 200) {
              ...
              return response
                .json()
                .then(
                  (res) => `Connection Error : ${res.errorType}: ${res.message}`
                )
                .then((errMsg) => Promise.reject(errMsg));
            } else {
          ...
          .then((userMetadata) =>
            this.setState({ userMetadata, connectionError: null })
          )
  ...
  renderNotConnected = () => {
    ...
    if (connectionError) {
      ...
      message =
        'To begin, enable "Developer Mode" from Extensions settings in the Skedulo Web App';
    }
    ...
    return (
      <div>
        ...
        <ButtonGroup>
          <Button
            buttonType="primary"
            onClick={this.setView(View.ManagePackages)}
          >
            Manage Package
          </Button>
          <Button
            buttonType="secondary"
            onClick={this.setView(View.LegacyManageConnectedPages)}
          >
            Manage Standalone Web Extensions
          </Button>
        </ButtonGroup>
      </div>
    );
    ...
    return (
      <div>
        ...
        <ButtonGroup>
          <Button
            buttonType="primary"
            onClick={this.setView(View.CreatePackage)}
          >
            Create new package
          </Button>
          <Button
            buttonType="secondary"
            onClick={this.setView(View.OpenPackage)}
          >
    ...
    return (
      <div>
        ...
        <ButtonGroup>
          <Button
            buttonType="primary"
            onClick={this.setView(View.LegacyCreateCPProject)}
          >
            Create new project
          </Button>
          <Button
            buttonType="secondary"
            onClick={this.setView(View.LegacySelectCPProject)}
          >
  ...
  renderDiagnostics = () => {
    return (
      <ContentLayout className="content__center--large" centered>
        <DebugInstall {...this.state.environment} setView={this.setView} />
      </ContentLayout>
    );
  };
      ...
      <>
        ...
        <div className="diagnostics-button">
          <IconButton
            ...
            icon="settings"
            onClick={this.setView(View.Diagnostics)}
            buttonType="secondary"
            tooltipContent="Diagnostics"
          />
        </div>
      </>
    );
  ...
  renderSSLHelpPrompt = () => {
    return (
      <ContentLayout centered>
        <h1>{SKEDULO_WELCOME_MESSAGE}</h1>
        <p>
          You need to setup self-signed SSL certificates to continue. Click{" "}
          <a className="blue-link" onClick={this.setView(View.SetupSSL)}>
            here
          </a>
  ...
  renderSSLHelpFromMarkdown = () => {
    return (
      <ContentLayout>
        ...
        <p>
          In order to run the Packages SDK, you will need to set up your dev
          environment with a self-signed SSL Certificate (must be X.509 v3) and
          follow a few steps to allow your Chrome browser to trust it. This is
          due to service workers requiring a trusted certificate to work
          correctly which are used during the development of your app.
        </p>
  ...
  renderUnsupportedPlatform = () => {
    return (
      <ContentLayout>
        <h1>{SKEDULO_WELCOME_MESSAGE}</h1>
        <p>
          Unfortunately, We do not support your platform at this time. We only
          support <b>OSX</b> and <b>Windows</b>
        </p>
      </ContentLayout>
    );
  };
  ...
  renderView = () => {
    ...
    switch (currentView) {
      ...
      case View.OpenPackage:
        ...
        return (
          <SelectPackage
            back={managePackageBack}
            session={session!}
            setPackage={trySetPackage}
          />
        );
      ...
      case View.LegacySelectCPProject:
        return (
          <SelectProject
            back={connectedPageBack}
            selectProject={this.selectConnectedPageProject}
      ...
      case View.LegacyActiveCPProject:
        return (
          <ActiveLegacyProject
            back={connectedPageBack}
            project={selectedLegacyProject!}
            session={session!}
          />
        );
      case View.LegacyCreateCPProject:
        return (
          <NewConnectedPageProject
            back={connectedPageBack}
            selectProject={this.selectConnectedPageProject}
          />
        );
